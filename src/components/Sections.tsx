...
export const PhilosophySection = () => {
  ...
  const isDefaultContent = !branding.greetingContent || 
                           ...
                           branding.greetingContent.trim() === '-' || 
                           branding.greetingContent === '대한민국 모든 보험사의 상품을 실시간으로 비교 분석하여 불필요한 고정 지출을 성공적으로 줄여 드립니다.';
        ...
        <div className="bg-white rounded-[4rem] p-12 md:p-20 shadow-xl relative border border-gray-100">
          ...
          <div className="relative z-10">
            {isDefaultTitle ? (
              ...
              <h2 className="text-3xl md:text-5xl font-black text-gray-900 mb-10 leading-tight break-keep">
                {branding.greetingTitle!.includes('안심') ? (
                  <>
                    {branding.greetingTitle!.split('안심')[0]}
                    <span className="text-orange-500">'안심'</span>
                    {branding.greetingTitle!.split('안심')[1]}
                  </>
                ) : (
            ...
            <div className="space-y-6 text-lg text-gray-600 leading-relaxed break-keep">
              {isDefaultContent ? (
                <>
                  <p>
                ...
                branding.greetingContent!.split('\n').map((para, idx) => (
                  <p key={idx}>{para}</p>
                ))
              )}
            </div>
            ...
            <div className="mt-16 flex items-center gap-6">
              <div className="w-20 h-20 bg-gray-200 rounded-3xl overflow-hidden shadow-lg transform -rotate-3">
                <img src={profileImg} alt="Planner" className="w-full h-full object-cover" referrerPolicy="no-referrer" />
              </div>
              <div>
                <p className="font-black text-2xl tracking-tight">{displayName}</p>
                <p className="text-gray-400 font-bold whitespace-pre-wrap">{displayCert}</p>
              </div>
            </div>
          </div>
        </div>
