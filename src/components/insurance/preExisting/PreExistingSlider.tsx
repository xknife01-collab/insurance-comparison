...
import { PreExistingExplanation } from './PreExistingExplanation';
    ...
    <section className="space-y-16">
      <div className="text-center space-y-6">
        <div className="inline-flex items-center gap-2 px-6 py-2 bg-indigo-950 text-white rounded-full text-[0.65rem] font-black uppercase tracking-[0.3em] shadow-xl border border-indigo-500/20">
          <Calculator size={14} className="text-indigo-400" /> Pre-Existing Simulation
        </div>
        <h2 className="text-4xl md:text-5xl font-black text-gray-900 tracking-tighter leading-tight">예산에 따른 보장 변화 시뮬레이션</h2>
      </div>

      <div className="bg-white rounded-[4rem] p-10 md:p-20 shadow-[0_40px_100px_-20px_rgba(0,0,0,0.06)] border border-indigo-100 relative overflow-hidden">
        <div className="grid lg:grid-cols-2 gap-20 items-center relative z-10">
          <div className="space-y-12">
            <div className="space-y-8">
              ...
              <div className="relative pt-10 pb-6">
                <input type="range" min={dietPremium} max={luxuryPremium} step={500} value={value} onChange={(e) => setValue(Number(e.target.value))} className="w-full h-3 bg-gray-100 rounded-full appearance-none cursor-pointer accent-indigo-500" />
                <div className="flex justify-between mt-6 text-[0.65rem] font-black text-gray-400 uppercase tracking-widest">
                  ...
                  <span className="text-indigo-500">현재 ({currentPremium.toLocaleString()}원)</span>
              ...
              <div className="flex gap-4">
                ...
                <div className="space-y-2">
                  <p className="text-[0.85rem] font-bold text-gray-600 leading-relaxed">
                    설정하신 월 예산에 맞춰 유병자 주요 질환 보장 한도가 <span className="text-indigo-600 font-black underline decoration-2 underline-offset-4">{metrics.benefit.toLocaleString()}만 원</span>으로 실시간 맞춤 조율되는 것을 확인하실 수 있습니다.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-slate-900 rounded-[3rem] p-10 text-white space-y-6 shadow-2xl border border-slate-800">
              ...
              <div className="h-1.5 bg-white/10 rounded-full overflow-hidden"><motion.div initial={{ width: 0 }} animate={{ width: `${(metrics.benefit / 7000) * 100}%` }} className="h-full bg-indigo-500" /></div>
            </div>
            <div className="bg-white rounded-[3rem] p-10 border border-gray-100 space-y-6 shadow-xl">
              ...
              <div>
                ...
                <p className="text-4xl font-black text-indigo-500 tracking-tighter">{metrics.index} <span className="text-xl text-gray-900">점</span></p>
              </div>
              <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden"><motion.div initial={{ width: 0 }} animate={{ width: `${metrics.index}%` }} className="h-full bg-indigo-500" /></div>
            </div>
          </div>
        </div>
        <PreExistingExplanation />
      </div>
    </section>
  );
};
