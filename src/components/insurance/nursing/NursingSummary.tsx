...
export const NursingSummary: React.FC<Props> = ({ result, forceMobile }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
        ...
        <div className="relative z-10 grid md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <h2 className="text-3xl md:text-4xl font-black mb-4 leading-tight">
              선택하신 맞춤 플랜의 <br/>
              <span className="text-pink-400">요양 보장 밸런스를 분석</span>했습니다.
            </h2>
      ...
      <div className="bg-gradient-to-r from-pink-600 to-rose-600 rounded-[2.5rem] p-10 text-white shadow-xl relative overflow-hidden">
        ...
        <div className={`relative z-10 flex ${forceMobile ? 'flex-col items-start' : 'flex-col md:flex-row md:items-center md:justify-between'} gap-8`}>
          <div>
            <span className="text-xs font-black text-pink-200 uppercase tracking-widest bg-pink-900/30 px-3 py-1 rounded-full mb-4 inline-block">
              Nursing Care Specialization Analysis
            </span>
            <h4 className="text-2xl font-black mb-2">동일 연령·조건 기준 월 예상 절약 보험료</h4>
            <p className="text-pink-100 text-sm font-bold opacity-80">
              사용자님의 연령과 장기요양 위험에 맞춘 최적의 재가·시설 플랜으로 전환 시 절감되는 금액입니다.
            </p>
          </div>
          <div className={forceMobile ? 'text-left' : 'text-right'}>
            {savings > 0 ? (
              <div className="flex items-baseline gap-2">
                ...
                <span className="text-xl font-bold text-pink-100">원 절감</span>
              </div>
            ) : (
              <div className="flex items-baseline gap-2">
            ...
            <p className="text-[10px] text-pink-200 font-bold mt-2 opacity-60 uppercase tracking-widest">
              * 추천 상품 기준 예상 수치
            </p>
          </div>
        </div>
