...
export const SilsonSlider: React.FC<{ result: AnalysisResult }> = ({ result }) => {
  ...
  const metrics = useMemo(() => {
    ...
    return {
      benefit: Math.round(1000 + (ratio * 4000)),
  ...
  return (
    <section className="space-y-16">
      <div className="text-center space-y-6">
        <div className="inline-flex items-center gap-2 px-6 py-2 bg-blue-900 text-white rounded-full text-[0.65rem] font-black uppercase tracking-[0.3em] shadow-xl">
          <Calculator size={14} className="text-blue-500" /> Silson Simulation
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-20 items-center relative z-10">
          <div className="space-y-12">
            <div className="space-y-8">
              ...
              <div className="relative pt-10 pb-6">
                <input type="range" min={dietPremium} max={luxuryPremium} step={500} value={value} onChange={(e) => setValue(Number(e.target.value))} className="w-full h-3 bg-gray-100 rounded-full appearance-none cursor-pointer accent-blue-500" />
                <div className="flex justify-between mt-6 text-[0.65rem] font-black text-gray-400 uppercase tracking-widest">
                  ...
                  <span className="text-blue-500">현재 ({currentPremium.toLocaleString()}원)</span>
              ...
              <div className="flex gap-4">
                ...
                <div className="space-y-2">
                  <p className="text-[0.85rem] font-bold text-gray-600 leading-relaxed">
                    설정하신 월 예산에 맞춰 비급여 치료비 예상 한도가 <span className="text-blue-600 font-black underline decoration-2 underline-offset-4">{metrics.benefit.toLocaleString()}만 원</span>으로 실시간 맞춤 조율되는 것을 확인하실 수 있습니다.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-slate-900 rounded-[3rem] p-10 text-white space-y-6 shadow-2xl">
              ...
              <div>
                <p className="text-[0.65rem] font-black text-slate-400 uppercase tracking-widest mb-1">비급여 예상 한도</p>
              ...
              <div className="h-1.5 bg-white/10 rounded-full overflow-hidden"><motion.div initial={{ width: 0 }} animate={{ width: `${(metrics.benefit / 5000) * 100}%` }} className="h-full bg-blue-500" /></div>
            </div>
            <div className="bg-white rounded-[3rem] p-10 border border-gray-100 space-y-6 shadow-xl">
              ...
              <div>
                ...
                <p className="text-4xl font-black text-blue-500 tracking-tighter">{metrics.index} <span className="text-xl text-gray-900">점</span></p>
              </div>
              <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden"><motion.div initial={{ width: 0 }} animate={{ width: `${metrics.index}%` }} className="h-full bg-blue-500" /></div>
            </div>
          </div>
        </div>
        <SilsonExplanation />
