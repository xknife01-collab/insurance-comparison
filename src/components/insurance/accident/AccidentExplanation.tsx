  ...
  <section className="py-24 bg-red-50/10 px-2 sm:px-4 relative overflow-hidden" id="accident-detail">
    <div className="max-w-7xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end mb-20 gap-8">
        ...
        <div className="max-w-md text-right hidden lg:block opacity-60">
          <p className="text-sm font-bold text-slate-500 leading-relaxed">
            상해 사망/후유장해는 물론, 깁스치료와 일상 낙상 사고까지!<br />
            직업 등급에 맞춘 합리적 설계와 야외 레저 특약 구성 가이드.
          </p>
        </div>
      </div>

      {/* ── [필수 고지] 예시 보장금액 산출 기준 및 계약자 유의사항 ── */}
      <div className="mb-16 bg-white rounded-3xl border border-red-200 p-6 md:p-8 shadow-sm">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 text-xs text-slate-600 space-y-2 leading-relaxed">
            ...
            <p className="font-bold text-slate-700">
              • **예시 산출 기준**: 주요 손해보험사 상해보험 표준 플랜 / 40세 남성 / 상해 1급(사무직) / 20년납 100세만기 / 월 보험료 18,500원 기준 (피보험자의 연령, 성별, 직업 위험도, 특약 선택 및 회사별 심사 기준에 따라 실제 보험료와 보장금액은 상이할 수 있습니다).
            </p>
            <div className="pt-2 border-t border-red-100 text-[11px] text-slate-500 space-y-1">
              <p>• **직업 위험도(급수) 주의사항**: 상해보험은 직업·직무 위험군(1급: 사무직, 2급: 외근/영업직, 3급: 현장/생산/배달직)에 따라 보험료 및 가입 한도가 크게 달라집니다.</p>
              <p>• **계약 후 알릴 의무 (통지의무)**: 가입 후 이직, 직무 변경, 오토바이(이륜차) 상시 운행 시 보험사에 즉시 서면 통지해야 하며, 미통지 상태에서 사고 발생 시 변경된 직업 요율에 따라 보험금이 비례 삭감되거나 계약이 해지될 수 있습니다.</p>
              <p>• **골절진단비 치아파절 여부**: 골절진단비는 회사별 약관에 따라 치아파절(치아 깨짐, S02.5) 제외 조건이 상이하므로 가입 전 약관의 제외 항목을 확인해야 합니다.</p>
              <p>• **상해후유장해 지급 기준**: 상해 사고로 영구적 장해가 남은 경우 약관상 장해분류표 기준 3%~100% 장해지급률에 따라 가입금액에 비례하여 차등 지급됩니다.</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 mb-16">
        {[
          { num: '비운전자 할인', label: '미운전 확인 시 요율 할인', sub: '회사별 인수 지침에 따라 5~10% 감면' },
          { num: '치아파절 선택특약', label: '골절진단비 치아 포함 여부', sub: '가입 전 약관의 제외 조항 확인 권장' },
          { num: '상해후유장해', label: '3% 이상 장해지급률별 지급', sub: '약관 기준 장해분류표에 따라 정액 보상' },
          { num: '레저 스포츠 특약', label: '등산/골프 등 야외 활동 케어', sub: '주요 상해 담보 선택 연계 가능' },
        ].map((s, i) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 mb-16">
        ...
        <div className="bg-white rounded-3xl md:rounded-[4rem] p-5 md:p-12 border border-red-100 shadow-lg hover:shadow-2xl transition-all group">
          ...
          <div className="space-y-3">
            {[
              { title: '사망/후유장해', label: '상해 사고로 인한 신체적 영구 장해 보장', color: 'bg-red-50/50 border-red-100', badge: 'text-red-700 bg-red-100', desc: '약관상 장해분류표 기준 3%~100% 장해지급률에 따라 가입금액에 비례하여 정액 지급되며, 소득 상실을 보완하는 핵심 담보' },
              { title: '골절/깁스 치료', label: '일상 낙상, 미끄러짐 및 골절 치료비 보강', color: 'bg-rose-50/50 border-rose-100', badge: 'text-rose-700 bg-rose-100', desc: '뼈가 깨지거나 금이 간 경우 지급되며, 통깁스치료 외에 반깁스 보장 여부 등 최신 특약 조건 확인 권장' },
              { title: '상해수술/입원', label: '사고로 인한 수술 및 입원 치료비 보장', color: 'bg-orange-50 border-orange-100', badge: 'text-orange-700 bg-orange-100', desc: '상해 사고로 수술 시 회당 정액 지급되며, 첫날부터 보장되는 상해입원일당을 추가하여 치료 기간 보장' },
            ].map((item, i) => (
          ...
          <div className="mt-8 p-5 md:p-6 bg-red-50 rounded-3xl border border-red-100">
            ...
            <p className="text-slate-700 font-bold text-xs leading-relaxed">
              상해보험은 가입 시 직업과 직무 위험군에 따라 요율이 결정되므로, 가입 후 **이직, 부서 이동, 주행 용도 변경(영업용/이륜차 사용 등)** 시 반드시 보험사에 변경 사실을 서면 통지해야 합니다. 불이행 시 사고 발생 시 변경된 직업 요율에 따라 보험금이 비례 삭감되거나 계약이 해지될 수 있습니다.
            </p>
          </div>
        </div>
        ...
        <div className="bg-slate-900 rounded-3xl md:rounded-[4rem] p-5 md:p-12 text-white shadow-2xl relative overflow-hidden group flex flex-col justify-between">
            ...
            <div className="space-y-6">
              <div className="p-5 md:p-6 bg-white/10 rounded-2xl md:rounded-[2.5rem] border border-white/10 hover:bg-white/15 transition-colors">
                ...
                <p className="text-xs opacity-75 font-bold leading-relaxed">
                  위험이 낮은 사무직·학생은 1급, 서비스·외근직은 2급, 제조업·현장직·배달원은 3급으로 분류됩니다. 급수가 올라갈수록 상해 위험에 비례하여 보험료가 높게 책정되며 가입 금액 한도가 축소될 수 있습니다.
                </p>
              </div>
              ...
              <div className="p-5 md:p-6 bg-red-600/20 rounded-2xl md:rounded-[2.5rem] border border-red-400/30 hover:bg-red-600/30 transition-colors">
                ...
                <p className="text-xs opacity-75 font-bold leading-relaxed mb-2">
                  대중교통만 이용하고 평소 운전을 전혀 하지 않는 경우 '비운전자' 요율을 적용받을 수 있습니다. 해당 특약 가입 시 운전자 대비 보험료 할인 혜택을 받을 수 있습니다 (회사별 인수 지침 상이).
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="relative z-10 mt-8 p-5 md:p-6 bg-white/5 rounded-3xl border border-white/10">
            ...
            <p className="text-white font-bold text-xs leading-relaxed opacity-80">
              "사고로 앞니가 깨지는 등 치아 파절(코드 S02.5)은 일상 상해 중 다발하는 재해입니다. 상품에 따라 '골절진단비(치아파절 제외)'와 '치아파절 포함' 특약이 구분되므로 가입 전 약관의 보장 범위를 꼭 확인해 보세요."
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-16 bg-gradient-to-br from-slate-900 to-slate-800 rounded-3xl md:rounded-[4rem] p-5 md:p-12 text-white relative overflow-hidden group">
        ...
        <div className="relative z-10 grid md:grid-cols-2 gap-12 items-center">
          <div>
            ...
            <h3 className="text-3xl font-black mb-4 tracking-tight">야외 스포츠 취미 인구와 전용 레저 특약</h3>
            <p className="text-sm opacity-70 font-bold leading-relaxed">
              등산, 러닝, 자전거 라이딩, 골프 등 아웃도어 스포츠의 보편화로 일상 및 취미 중 일어나는 근골격계 상해 사고 위험이 높아지고 있습니다. 합리적인 레저 특약을 연계하여 치료 한도를 넓히는 것이 유리합니다.
            </p>
          </div>
          <div className="space-y-4">
            {[
              { company: '레저 스포츠 상해', product: '등산/골프/자전거 사고 보강', limit: '골절 및 깁스비 추가 지원', note: '취미 활동 중 발생한 물리 충격 골절 및 외상 수술 케어' },
              { company: '주말 상해 특약', product: '휴일 교통/재해 특약', limit: '약정 가입금액 추가 지급', note: '주말 야외 나들이 차량 통행 중 돌발 재해에 특화 설계' },
              { company: '비운전자 할인 특약', product: '대중교통 이용자 요율 혜택', limit: '회사별 기준에 따라 감면', note: '소유 차량 유무와 무관하게 실질 운전을 하지 않는 피보험자 우대' },
            ].map((item, i) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-20">
        <div className="md:col-span-2 bg-white border border-red-100 rounded-3xl md:rounded-[4rem] p-5 md:p-12 shadow-sm hover:shadow-xl transition-all">
          ...
          <div className="space-y-3">
            {[
              { step: '01. 직무 변경 통지', desc: '이직이나 위험 업무 전환 시 보험사에 통지 의무를 이행했는지 확인하여 지급 분쟁 미연 방지' },
              { step: '02. 치아파절 보장 대조', desc: '골절 특약 중 치아 제외 문구가 포함되어 있는지 약관 확인 및 포함 조건 비교' },
              { step: '03. 후유장해 3% 이상', desc: '80% 이상 고도장해 조건뿐 아니라 3% 이상 장해분류표 전 구간 보강 여부 확인' },
              { step: '04. 레저/주말 특약 추가', desc: '레저 취미 생활을 즐기는 경우 주말/특정 상해 추가로 보장 범위 확대 검토' },
            ].map((item, i) => (
        ...
        <div className="flex flex-col gap-6">
          <div className="bg-red-600 text-white rounded-3xl md:rounded-[3.5rem] p-6 md:p-10 shadow-xl">
            <h4 className="text-xl font-black mb-4">상해 리모델링 핵심 5대 점검사항</h4>
            <p className="text-xs font-bold opacity-90 leading-relaxed">
              ① 이직 시 통지의무 누락 확인<br />
              ② 골절진단비 치아파절 포함 여부 체크<br />
              ③ 상해후유장해(3%~100%) 적정 한도 확보<br />
      ...
      <div className="mb-20 bg-white rounded-3xl md:rounded-[4rem] p-5 md:p-12 border border-red-100 shadow-sm">
        <h3 className="text-2xl font-black text-slate-900 mb-10 tracking-tight">
          주요 손해보험사 상해보험 상품별 특징 안내 (손해보험협회 공시 기준)
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {[
            ...
            { company: '현대해상', product: '무배당 현대해상 상해케어', highlight: '레저/스포츠 특약 다양화, 자전거/골프 주말 돌발 사고 추가 지급 조건 우수', badges: ['레저스포츠 강자', '주말 나들이 특화'] },
            { company: 'DB손해보험', product: '참좋은 상해보험', highlight: '직업 위험군별 가입 한도 적용, 중대 후유장해 시 가계 안전을 위한 연금식 분할 지급 지원', badges: ['현장직 인수 우대', '생활자금 옵션'] },
            { company: 'KB손해보험', product: 'KB 다이렉트 상해보험', highlight: '일상 미끄러짐 및 단순 골절/깁스 정액 진단비 요율 매칭, 비운전자 할인 혜택 우수', badges: ['골절/깁스 강점', '비운전 할인 우수'] },
            { company: 'Meritz 메리츠화재', product: '메리츠 올바른 상해보험', highlight: '2~3급 직무 인수 기준 유연성, 다발 사고에 대한 모바일 간편 청구 및 보상금 신속 지급 연동', badges: ['3급 인수 유연', '신속 보상 강자'] },
          ].map((item, i) => (
      ...
      <div className="border-t border-red-100 pt-20 flex flex-col md:flex-row justify-between items-center gap-10">
        <div className="flex items-center gap-6">
          ...
          <p className="text-2xl font-black text-slate-900 tracking-tight leading-tight">
            ...
            <span className="text-red-600">균형 잡힌 고지와 객관적인 비교 진단을 통해 언제 발생할지 모르는 일상 리스크에 든든한 방패를 세워 드립니다.</span>"
          </p>
        </div>
        {onAction && (
          <button
