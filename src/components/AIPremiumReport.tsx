...
export const AIPremiumReport: React.FC<AIPremiumReportProps> = ({ analysis, deficiencies, scores, forceMobile }) => {
  ...
  const getReportData = () => {
    ...
    // 치아 보험 팁
      ...
      riskTitle = `🦷 치과 치료비 부담 완화 - 보철 및 보존 집중 가이드`;
      riskPercent = 65;
      riskStats = `치과 진료는 국민건강보험의 비급여 항목 비율이 높아 개인이 부담하는 비용이 클 수 있습니다. 특히 영구치 보존 치료나 임플란트 시술은 고액의 비용이 발생할 수 있으므로 치아 건강 상태와 치료 계획에 따른 맞춤형 플랜 준비가 권장됩니다.`;
      goodPoints = [
        '치료 한도가 큰 보철 특약을 검토 중이시며, 최근 구강 정밀 진단 내역이 양호하여 가입 조건이 유연합니다.'
      ];
      badPoints = [
        '충치 치료(레진, 인레이) 같은 다빈도 치료는 보장 횟수나 연간 한도가 정해져 있을 수 있어 세부 조건을 확인할 필요가 있습니다.'
      ];
      actionTips = [
        '향후 임플란트 등 보철 치료 계획이 있다면 연간 한도가 넉넉한 "보철 집중 플랜"을 선택하고, 일상 충치 치료가 목적이라면 비용이 합리적인 "보존 중심 플랜"으로 조율하세요.',
        '치과 치료는 질병 가입 시 90일간 면책기간 및 1~2년 감액기간이 적용될 수 있으므로, 치료 계획을 세우기 전 보장 개시 시점을 미리 확인하시는 것이 유리합니다.'
      ];
    }
    // 간병/요양 보험 팁
