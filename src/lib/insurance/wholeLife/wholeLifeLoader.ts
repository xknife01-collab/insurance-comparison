...
export const fetchWholeLifePremium = async (analysis: InsuranceAnalysis): Promise<any> => {
  ...
  // 2. DB 데이터 없을 시 프리미엄 폴백 Mock 리스트 매핑
  ...
  const results = productsList.map(p => {
    ...
    const premium = Math.round((baseUnitPremium * ageFactor * genderFactor * amountFactor * periodFactor * refundFactor * stepUpFactor) / 100) * 100;
  ...
  const filtered = results.filter(r => {
    const isProdLow = r.productName.includes('저해지') || r.productName.includes('알뜰한');
