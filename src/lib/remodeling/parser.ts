...
가입자의 기본 정보(나이: ${age}세, 성별: ${gender === 'M' ? '남성' : '여성'})와 가입된 보험 목록(상품명, 월 보험료)을 기반으로, 국내 표준 설계 요율 및 통계 자료를 바탕으로 각 상품별 핵심 특약(일반암 진단비, 뇌혈관질환 진단비, 허혈성심장질환 진단비, 간병인사용일당, 실손의료비 가입여부)의 보장 금액을 정교하게 역산하여 추정해 주세요.
...
4. 각 보험의 상품명과 월 보험료 수준을 고려하여 현실적인 데이터로 역산해야 합니다. 예를 들어, 암보험이면 암 진단비가 높을 것이고, 건강/종합보험이면 암, 뇌, 심장 진단비가 고루 들어있을 것입니다. 운전자보험이나 화재보험 등 기타 보험은 해당 특약들이 없을 것이므로 0으로 설정하세요.
