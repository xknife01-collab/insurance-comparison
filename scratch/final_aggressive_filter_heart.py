...
def final_aggressive_filter_heart():
    ...
    for filename in files:
        # 파일명 자체에 종신/변액/연금이 있으면 스킵
        if any(ex in filename.lower() for ex in ["file_0", "file_1", "file_2", "file_3", "file_4", "file_5", "file_6", "file_7", "file_8", "file_9", "life", "variable"]):
            continue
        ...
        try:
            ...
            # 상품명 필터 한번 더
            if any(ex in real_prod_name for ex in ["종신", "변액", "연금"]):
                continue
    ...
    if all_data:
        ...
        print(f"SUCCESS: {len(df_final)} PURE HEALTH records saved. ALL LIFE INSURANCE REMOVED.")
    else:
        print("No pure health insurance data found.")

if __name__ == "__main__":
    final_aggressive_filter_heart()
