...
const COMPANIES = ['DB손해보험','KB손해보험','한화손해보�function detectType(name: string) {
...
function extractAllCov(riders: Rider[]) {
  ...
  return {
    ...
    dementiaMild, dementiaModerate, dementiaSevere,
    ...
    petPatella, petSkin, petDental, petLiability,
        ...
        {open&&(
          <motion.div initial={{height:0,opacity:0}} animate={{height:'auto',opacity:1}} exit={{height:0,opacity:0}} transition={{duration:0.3}} className="overflow-hidden">
            <div className="border-t border-slate-100 px-6 md:px-8 pb-8 space-y-8 pt-6">
              ...
              </div>re > 0 ? fmt(cov.accidentFracture) : '미가입'} status={statusOf(cov.accidentFracture, 500000, 300000)} note="권장: 30~50만원"/>
                    <CovRow label="상해수술비 / 입원비" value={cov.accidentSurgery > 0 ? fmt(cov.accidentSurgery) : '미가입'} status={statusOf(cov.accidentSurgery, 1000000, 500000)} note="권장: 상해 치료 보강"/>
                  </div>
                </div>
              )}
