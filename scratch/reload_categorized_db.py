...
if os.path.exists(env_path):
    with open(env_path, 'r') as f:
        for line in f:
            if 'NEXT_PUBLIC_SUPABASE_URL' in line: url = line.split('=')[1].strip()
            if 'NEXT_PUBLIC_SUPABASE_ANON_KEY' in line: key = line.split('=')[1].strip()
...
exclude_patterns = ['종신', 'CEO', '경영인', '어린이', '태아', '자녀', '유병자', '간편', '325', '335', '355']
...
for file in all_files:
    ...
    file_data = {} # (comp, prod) -> {cat_1_5: {m,f}, cat_n: {m,f}, ...}
    ...
    for (comp, prod), cats in file_data.items():
        # Sum up total male to check filter
        total_m = sum(c['m'] for c in cats.values()) / divisor
        if total_m < MIN_PREMIUM: continue
        
        processed_cats = {}
        for c_name, c_data in cats.items():
            m_raw = int(c_data['m'] / divisor)
            f_raw = int(c_data['f'] / divisor)
            # Gender ratio for missing data
            if m_raw > 0 and f_raw == 0: f_raw = int(m_raw * RATIO)
            elif f_raw > 0 and m_raw == 0: m_raw = int(f_raw / RATIO)
            processed_cats[c_name + '_m'] = m_raw
            processed_cats[c_name + '_f'] = f_raw

        final_key = (comp, prod)
        if final_key not in products_dict or total_m > products_dict[final_key]['total_m']:
...
# Note: Ensure the table has these columns: cat_1_5_m, cat_1_5_f, cat_n_disease_m, cat_n_disease_f, cat_injury_m, cat_injury_f, cat_hospital_m, cat_hospital_f, cat_caregiver_m, cat_caregiver_f
final_items = []
for p in products_dict.values():
    item = {k: v for k, v in p.items() if k != 'total_m'}
    final_items.append(item)

print(f"Uploading {len(final_items)} products with 5-category premiums...")
...
print("DATABASE RE-STRUCTURED AND RELOADED.")
