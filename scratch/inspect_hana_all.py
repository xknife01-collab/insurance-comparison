...
# Find rows where Col 1 is '하나손보' or ffilled
current_company = ""
current_product = ""
matching_rows = []
...
for idx, row in df.iterrows():
    ...
    if "교직원 안심보험" in current_product:
        matching_rows.append((idx, current_company, current_product, row.tolist()))

with open("scratch/inspect_hana_all.txt", "w", encoding="utf-8") as out:
...
print(f"Hana rows written. Total: {len(matching_rows)}")
