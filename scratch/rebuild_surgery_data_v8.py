...
# Exclusion keywords (Focus on removing Whole Life/Annuity only)
...
for file in all_files:
        ...
        for enc in ['utf-8-sig', 'cp949', 'utf-8']:
            try:
                with open(file, 'r', encoding=enc, errors='ignore') as f:
                    ...
                    if '<table' in test_content.lower():
    ...
    for tr in rows:
        ...
        # --- EXCLUSION FILTER (NO PREMIUM CAP) ---
        full_text = (current_company + " " + product_name + " " + coverage_name).replace(" ", "")
        
        # Strictly remove whole life/annuity keywords
        if any(k in full_text for k in exclude_keywords): continue
        # Must have surgery/hospitalization keywords
...
print(f"\nFinal INCLUSIVE Surgery Extraction: {len(final_combined)} products (No Premium Cap).")
