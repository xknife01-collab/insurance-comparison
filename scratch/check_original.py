...
# The old diet card block (motion.div from line 676 to 735)
# We'll replace it with a new structure that wraps everything in a single card
# AND adds the isRemodeling-aware layout
...
if old in content:
    print('Found old block - no changes needed (already original)')
else:
    print('Old block NOT found in content')
    # find the diet section
    idx = content.find('{/* Diet Type */}')
    print(f'Diet Type comment at char index: {idx}')
