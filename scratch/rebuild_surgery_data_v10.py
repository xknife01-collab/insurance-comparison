...
for file in all_files:
    ...
    # Try CP949 first as it's most likely for Korean Excel exports
    ...
    for tr in rows:
        ...
        # 2. Broken text check (if '보험' or '수술' isn't readable, skip)
        if '' in full_text: continue 
        # 3. Must be surgery/hospital
        ...
        # 4. Premium Cap for sanity (Over 500k is almost never surgery insurance)
        if m_prem > 500000 or f_prem > 500000: continue
...
print(f"\nSUCCESS: {len(final_combined)} clean products extracted.")
