...
for file in all_files:
    ...
    # --- FILE-LEVEL DIVIDOR DETECTION ---
    # Scan the entire file content for payment terms
    divisor = 1
    # Check for X년납 in the whole file
    match_yr = re.findall(r'(\d+)년납', content)
    if match_yr:
        # Take the most frequent or the smallest common one? Usually it's one per file.
        years = int(match_yr[0])
        divisor = years * 12
    elif '일시납' in content or '일시' in content:
        divisor = 12
    
    # Special override for known high-premium cases (KDB, iM)
    # If the file contains huge numbers but doesn't explicitly say 3년납 in a way regex catches
    # (Though we saw 3년납 in the dump, so regex should catch it)
    ...
    for tr in rows:
        ...
        # Only apply divisor if the raw premium is high (safety check)
        # If it's 27,000,000, dividing by 36 makes it ~750,000.
        current_divisor = divisor
        if m_raw < 5000: # If it's already small, don't divide (could be a monthly rider)
            current_divisor = 1
            
        m_norm = int(m_raw / current_divisor) if current_divisor > 0 else m_raw
        f_norm = int(f_raw / current_divisor) if current_divisor > 0 else f_raw
...
print(f"\nSUCCESS: {len(final_combined)} products normalized by file-level divisor.")
