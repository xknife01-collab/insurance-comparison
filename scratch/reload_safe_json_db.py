...
exclude_patterns = ['종신', 'CEO', '경영인', '어린이', '태아', '자녀', '유병자', '간편']
...
for file in all_files:
    ...
    for (comp, prod), cats in file_data.items():
        ...
        # Format categories as JSON metadata to avoid schema errors
        processed_cats = {}
        for c_name, c_data in cats.items():
            ...
            processed_cats[c_name] = {'m': m_raw, 'f': f_raw}

        final_key = (comp, prod)
        if final_key not in products_dict or total_m > products_dict[final_key]['premium_male']:
            products_dict[final_key] = {
                'company_name': comp,
                'product_name': prod,
                'premium_male': int(total_m),
                'premium_female': int(total_m * RATIO),
                'coverage_details': json.dumps(processed_cats), # Store all detailed categories here
                'age': 40
            }

final_items = list(products_dict.values())
print(f"Uploading {len(final_items)} products with JSON metadata...")
...
print("DB RELOAD WITH JSON METADATA COMPLETE.")
