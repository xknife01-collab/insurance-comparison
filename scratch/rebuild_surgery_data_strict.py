...
for file in all_files:
    ...
    # --- STRICT DATA-DRIVEN DIVISOR DETECTION ---
    divisor = 1
    term_text = ""
    
    # Search all table rows for "납입기간" or "보험료 납입기간"
    for tr in soup.find_all('tr'):
        cells = [clean_text(c.get_text()) for c in tr.find_all(['td', 'th'])]
        for i, c in enumerate(cells):
            if '납입기간' in c and '보험' in c or ('납입기간' in c and len(c) < 10):
    ...
    # If not found in labeled cells, search the whole text for specific patterns
    if not term_text:
        all_text = soup.get_text()
        # Look for "납입기간 : X년" or "X년납"
    ...
    if term_text:
        if '일시납' in term_text:
            ...
            # Extract number before '년'
            match_yr = re.search(r'(\d+)\s*년', term_text)
            if match_yr:
                years = int(match_yr.group(1))
                if years <= 5: # Only normalize short terms (1, 3, 5 years)
                    divisor = years * 12
        if divisor > 1:
            print(f"[{file_name}] Data Found: '{term_text}' -> Normalizing by {divisor}")
    ...
    for tr in rows:
        ...
        # Apply the divisor found strictly from the file
...
print(f"\nSUCCESS: {len(final_combined)} products processed STICKY TO DATA.")
