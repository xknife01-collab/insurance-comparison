...
def final_strict_heart_extraction():
    ...
    for filename in files:
        ...
        try:
            ...
            if not real_prod_name:
                candidates = [t.get_text(strip=True) for t in soup.find_all(['th', 'td', 'div', 'p', 'h1'])]
                for t in candidates:
    ...
    if all_data:
        ...
        print(f"SUCCESS: {len(df_final)} heart insurance records (Excluding Cancer/WholeLife) saved.")
    else:
        print("No specialized heart insurance data found.")

if __name__ == "__main__":
    final_strict_heart_extraction()
