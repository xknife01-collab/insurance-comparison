...
def final_heart_comprehensive_extraction():
    ...
    for filename in files:
        ...
        try:
            ...
            # --- 상품명 추출 ---
            prod_name = ""
            text_elements = [t.get_text(strip=True) for t in soup.find_all(['th', 'td', 'caption', 'h1', 'h2', 'div', 'p'])[:50]]
            ...
            if prod_name:
                prod_name = prod_name.replace("상품명", "").replace(":", "").strip()
            
            # --- [필터링 핵심] ---
            # 1. '암보험'(암전용) 또는 '종신보험'은 제외
            # 단, '건강보험'이나 '종합보험'은 암 보장이 섞여있어도 심장 보장이 중요하므로 포함
            if any(ex in prod_name for ex in ["암보험", "종신", "변액", "연금"]):
                continue
    ...
    if all_data:
        ...
        print(f"SUCCESS: {len(df_final)} comprehensive heart records saved.")
    else:
        print("No heart records found.")

if __name__ == "__main__":
    final_heart_comprehensive_extraction()
