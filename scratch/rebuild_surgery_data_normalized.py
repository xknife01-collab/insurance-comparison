...
for file in all_files:
    ...
    for tr in rows:
        ...
        # Search for payment term keywords in ALL columns of this row
        all_row_text = " ".join(cell_texts)
        divisor = 1
        if '일시납' in all_row_text or '일시' in all_row_text:
            divisor = 12
        elif '1년납' in all_row_text:
            divisor = 1 # Usually 1yr pay is monthly for 1 yr, but if it's a huge total, we might need 12. 
            # Given the 27M won case, it's clearly a lump sum.
            if m_prem > 500000: divisor = 12
        
        if divisor > 1:
            m_prem = int(m_prem / divisor)
            f_prem = int(f_prem / divisor)
            cell_texts[male_idx] = f"{m_prem:,} 원 (월환산)"
            if female_idx != -1: cell_texts[female_idx] = f"{f_prem:,} 원 (월환산)"
...
print(f"\nSUCCESS: {len(final_combined)} normalized products extracted.")
