...
for file in all_files:
    ...
    for (comp, prod), cats in file_data.items():
        total_m_raw = sum(c['m'] for c in cats.values()) / divisor
        if total_m_raw < MIN_PREMIUM: continue
        
        processed_data = {
            'company_name': comp,
            'product_name': prod,
            'premium_male': int(total_m_raw),
            'premium_female': int(total_m_raw * RATIO),
            'age': 40
        }
        ...
        for c_name, c_data in cats.items():
            ...
            elif f_norm > 0 and m_raw == 0: m_raw = int(f_raw / RATIO) # Fixed typo here
            
            processed_data[c_name + '_m'] = m_norm
            processed_data[c_name + '_f'] = f_norm

        final_key = (comp, prod)
        if final_key not in products_dict or total_m_raw > products_dict[final_key]['premium_male']:
