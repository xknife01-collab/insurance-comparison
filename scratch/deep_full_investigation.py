...
for file in all_files:
    ...
    # Check if this file/product should be excluded entirely
    if any(k in content for k in exclude_patterns):
        # We also check the filename for safety
        if any(k in file_name for k in exclude_patterns):
            continue
    ...
    for tr in rows:
        ...
        # Check exclusion again for the specific product/company name found in table
        ...
        if key not in product_data:
            product_data[key] = {'m': 0, 'f': 0, 'has_relevant_coverage': False}

        # Check for surgery/hospitalization anywhere in the row
        row_text = " ".join(cell_texts)
        if any(k in row_text for k in include_keywords):
            product_data[key]['has_relevant_coverage'] = True
    ...
    for (comp, prod), data in product_data.items():
        if not data['has_relevant_coverage']: continue
...
results.sort(key=lambda x: x['male'])

print(f"{'Company':<15} | {'Product':<50} | {'Male (Won)':<15} | {'Female (Won)':<15}")
print("-" * 100)
for r in results:
    print(f"{r['comp']:<15} | {r['prod'][:50]:<50} | {r['male']:,} 원 | {r['female']:,} 원")
