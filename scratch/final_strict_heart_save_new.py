...
def final_strict_heart_save_new():
    parent_dir = r"C:\Users\zkfnt\Desktop\insurance-comparison-main"
    # 충돌 방지를 위해 파일명 변경
    save_path = r"C:\Users\zkfnt\Desktop\insurance-comparison-main\insurance-comparison-main\insurance_data\1_guaranteed\heart\heart_master_final_perfect.csv"
    ...
    for filename in files:
        ...
        try:
            ...
            real_prod_name = ""
            candidates = [t.get_text(strip=True) for t in soup.find_all(['th', 'td', 'caption', 'h1'])]
            for t in candidates:
            ...
            for row in rows:
                ...
                if any(k in text for k in ["심장", "허혈", "혈관", "부정맥", "심근", "뇌출혈", "뇌졸중"]):
                    if any(ex in text for ex in ["암진단", "암수술"]): continue
    ...
    if all_data:
        ...
        print(f"COMPLETE: {len(df_final)} records saved to {save_path}. (Cancer/WholeLife excluded)")
    else:
        print("No specialized heart insurance data found.")

if __name__ == "__main__":
    final_strict_heart_save_new()
