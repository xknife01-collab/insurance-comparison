...
for file in all_files:
    ...
    for tr in rows:
        ...
        # 2. Premium Cap (Surgery/Hospitalization shouldn't be over 200k in 99% of cases)
        m_prem = get_premium_val(cell_texts[male_idx])
        f_prem = get_premium_val(cell_texts[female_idx]) if female_idx != -1 else 0
        if m_prem > 200000 or f_prem > 200000: continue
...
# One more pass: Combine Main + 1 Rider for the final clean list
final_combined = []
products_processed = {}
...
for row in final_rows:
    ...
    if pname not in products_processed:
        ...
        # Add rider premium to existing product base
...
print(f"\nFinal PURE Surgery Extraction: {len(final_combined)} products.")
