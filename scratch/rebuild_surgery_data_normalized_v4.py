...
for file in all_files:
    ...
    # --- ADVANCED DIVIDOR DETECTION ---
    divisor = 1
    
    # 1. Look for explicit patterns
    if '일시납' in content or '일시' in content:
        divisor = 12
    elif '3년납' in content or '3년' in content:
        divisor = 36
    elif '5년납' in content or '5년' in content:
        divisor = 60
    elif '1년납' in content or '1년' in content:
        divisor = 12
        
    # 2. Heuristic for KDB/iM high premium files
    # If the file doesn't say "3년납" clearly but has huge numbers, and "3" appears in descriptions
    # We saw Column 1804 had 3년납 info.
    ...
    for tr in rows:
        ...
        # HEURISTIC: If m_raw > 1,000,000 and divisor is 1, force check for '3' or '5'
        cur_div = divisor
        if m_raw >= 1000000 and cur_div == 1:
            if '3' in content: cur_div = 36
            elif '5' in content: cur_div = 60
            else: cur_div = 12 # Fallback to single pay if unknown high premium
            
        m_norm = int(m_raw / cur_div) if cur_div > 0 else m_raw
        f_norm = int(f_raw / cur_div) if cur_div > 0 else f_raw
...
print(f"\nSUCCESS: {len(final_combined)} products normalized with heuristic (3yr->/36).")
