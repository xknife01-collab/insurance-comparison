...
RATIO = 1.36
MIN_PREMIUM = 20000 # Keep products >= 20,000 Won

# DEFINITIVE Exclusion Patterns
exclude_patterns = [
...
for file in all_files:
    ...
    for (comp, prod), cats in file_data.items():
        ...
        # USER SPECIAL REQUEST: If >= 500k, divide by 12 (assuming annual)
        internal_divisor = 1
        if total_m_val >= 500000:
            internal_divisor = 12
            total_m_val = total_m_val / 12
        ...
        for c_name, c_data in cats.items():
            ...
            # Apply both divisors
            m_norm = int(c_data['m'] / (divisor * internal_divisor))
            f_norm = int(c_data['f'] / (divisor * internal_divisor))
...
print(f"Uploading {len(final_items)} products (>= 20,000 Won, 500k+ corrected) to Supabase...")
...
print("NORMALIZED DB RELOAD COMPLETE.")
