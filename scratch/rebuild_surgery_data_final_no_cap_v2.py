...
# Exclusion keywords (STRICTLY NO PREMIUM CAP)
...
for file in all_files:
    ...
    # --- INDIVIDUAL FILE ENCODING DETECTION ---
    success_enc = None
    # Try common encodings one by one for EACH file
    for enc in ['utf-8', 'cp949', 'utf-8-sig', 'euc-kr']:
        try:
            with open(file, 'r', encoding=enc) as f:
                temp = f.read()
                # If we can see '보험' correctly, this is the right encoding
    ...
    if not content:
        # Last resort: read with ignore errors
        try:
