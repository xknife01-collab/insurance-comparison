...
for file in all_files:
    ...
    for enc in ['cp949', 'utf-8-sig', 'utf-8', 'euc-kr']:
        try:
            with open(file, 'r', encoding=enc) as f:
                temp_content = f.read()
                # Check if common Korean insurance words are readable
    ...
    if not content:
        # Fallback if no encoding worked perfectly
        try:
...
print(f"\nFINAL REBUILD SUCCESS: {len(final_combined)} products.")
print(f"Sample product from result: {final_combined[0][1] if final_combined else 'None'}")
