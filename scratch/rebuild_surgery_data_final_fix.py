...
# --- NORMALIZE LAST (POST-MERGE) ---
final_combined = list(products_processed.values())
for row in final_combined:
    m_total = get_premium_val(row[7])
    f_total = get_premium_val(row[8])
    ...
    if divisor > 1:
        m_norm = int(m_total / divisor)
        f_norm = int(f_total / divisor)
        row[7] = f"{m_norm:,} 원 (환산)"
        row[8] = f"{f_norm:,} 원 (환산)"
...
print(f"\nSUCCESS: {len(final_combined)} products POST-MERGE NORMALIZED (1M+ -> /36).")
