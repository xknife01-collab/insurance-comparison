...
file_path = r"c:\Users\zkfnt\Desktop\insurance-comparison-main\insurance-comparison-main\src\components\AnalysisDashboard.tsx"

with open(file_path, 'r', encoding='utf-8') as f:
    content = f.read()

# Define the target block to search for
target = """             <ul className="space-y-6 flex-1 mb-12">
...
replacement = """             <ul className="space-y-6 flex-1 mb-12">
               {result.recommendations.upgrade.coverageChanges.map((change, i) => (
                 <li key={i} className="flex items-center gap-4 text-sm font-bold">
                    <div className="w-6 h-6 rounded-full bg-orange-500/20 flex items-center justify-center flex-shrink-0">
                      <ShieldCheck className="w-4 h-4 text-orange-500" />
...
if target in content:
    ...
    with open(file_path, 'w', encoding='utf-8') as f:
        f.write(content)
    print("SUCCESS: Successfully inserted the Upgrade table!")
else:
    # Try with single-spaced indentation
    target_clean = target.replace('\r\n', '\n')
    content_clean = content.replace('\r\n', '\n')
    if target_clean in content_clean:
        content_clean = content_clean.replace(target_clean, replacement.replace('\r\n', '\n'))
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(content_clean)
        print("SUCCESS: Successfully inserted the Upgrade table (with line ending normalization)!")
    else:
        # Let's print some lines to see spacing
        print("FAIL: Could not find target in content. Printing file slice:")
        idx = content.find("recommendations.upgrade.coverageChanges.map")
        if idx != -1:
            print(content[idx-100:idx+300])
        else:
            print("Could not find substring at all")
