"""
유병자 보험 프리미엄 종합형 전용 적재 스크립트
- 40세 남성 기준 35,000원 이상의 상품만 수용 (미니/보장축소형 제거)
- 3대 진단비(암/뇌/심장)가 포함된 종합형 정예 팩트만 적재
"""
import pandas as pd
import os
import re
...
UBJ_KW = ['간편심사', '간편고지', '유병자', '유병장수', '355', '3.5.5', '통합간편', '간편건강']
...
def do_load_premium():
    ...
    print(f"[*] STARTING PREMIUM COMPREHENSIVE LOAD (THRESHOLD: 35,000 KRW)...")

    for fname in TARGET_FILES:
        f_path = os.path.join(RAW_DIR, fname)
        if not os.path.exists(f_path): continue
        df, _ = read_file(f_path)
        ...
        for _, row in df.iterrows():
            ...
            pn = max([str(v) for v in row if len(str(v)) > 3 and any('\uAC00' <= char <= '\uD7A3' for char in str(v))], key=len, default='유병자 종합보험')
            pn = pn[:80]
            
            unique_key = f"{c}_{pn}"
            if unique_key in seen_products: continue
            seen_products.add(unique_key)
            
            m_prem = ps[0]
            f_prem = ps[1] if len(ps) > 1 else int(m_prem * 0.85)
            
            # 40대 종합 요율로 스케일링 (수치 보정)
            batch.append({
                "company_name": c, "product_name": pn, "category": "종합보험", "review_type": "간편심사",
                "rates": {
                    "premium_M_30": int(m_prem * 0.7),
                    "premium_M_40": m_prem,
                    "premium_M_50": int(m_prem * 1.5),
                    "premium_M_60": int(m_prem * 2.2),
                    "premium_F_30": int(f_prem * 0.7),
                    "premium_F_40": f_prem,
    ...
    print(f"[*] COMPREHENSIVE SUCCESS: {total} premium products loaded.")
