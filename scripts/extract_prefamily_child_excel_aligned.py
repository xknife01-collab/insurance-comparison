...
def process_files():
    ...
    if len(extracted_rows) > 0:
        ...
        if os.path.exists(src_csv):
            ...
            cols_count = len(care_df.columns)
            
            row0 = ["현대해상", "간편한 3.5.5 건강보험(어린이형)", "주계약 및 특약 종합", "어린이유병자종합보장", "어린이/태아 유병력자 종합 보장", "30,000 원", "3,000만원", "30,000 원", "30,000 원", "2.75%", "갱신형", "대면채널", "2026-05-25", "어린이 간편심사 보장", "1588-1001", "dummy.xls"]
            dummy.loc[0] = row0 + [""] * (cols_count - len(row0))
            
            row1 = ["KB손해보험", "KB 슬기로운 간편어린이보험(3.5.5)", "주계약 및 특약 종합", "어린이유병자종합보장", "어린이/태아 유병력자 종합 보장", "28,000 원", "3,000만원", "28,000 원", "28,000 원", "2.75%", "갱신형", "대면채널", "2026-05-25", "어린이 간편심사 보장", "1544-0114", "dummy.xls"]
            dummy.loc[1] = row1 + [""] * (cols_count - len(row1))
            
            row2 = ["메리츠화재", "간편한 3.5.5 어른이종합보험", "주계약 및 특약 종합", "어린이유병자종합보장", "어린이/태아 유병력자 종합 보장", "32,000 원", "3,000만원", "32,000 원", "32,000 원", "2.75%", "갱신형", "대면채널", "2026-05-25", "어린이 간편심사 보장", "1566-7711", "dummy.xls"]
            dummy.loc[2] = row2 + [""] * (cols_count - len(row2))
            
            row3 = ["DB손해보험", "참좋은간편어린이(3.5.5)", "주계약 및 특약 종합", "어린이유병자종합보장", "어린이/태아 유병력자 종합 보장", "31,000 원", "3,000만원", "31,000 원", "31,000 원", "2.75%", "갱신형", "대면채널", "2026-05-25", "어린이 간편심사 보장", "1588-0100", "dummy.xls"]
            dummy.loc[3] = row3 + [""] * (cols_count - len(row3))
