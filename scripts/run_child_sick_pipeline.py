    ...
    for root in search_roots:
        ...
        for name in os.listdir(root):
            ...
            if os.path.isdir(full_path) and name not in ignore_folders:
                ...
                if len(xls_files) > 5:  # At least 5 files to qualify as a disclosure dump
                    return full_path
    return None
...
def update_disclosure_dates():
    print(f"\n[*] Updating child sick disclosure metadata in {METADATA_JSON_PATH}...")
    try:
        now = datetime.now()
        ...
        data["child"] = current_disclosure_str
        data["preexisting"] = current_disclosure_str
...
def main():
    print("==================================================")
    print("      CHILD SICK INSURANCE PIPELINE RUNNER        ")
    print("==================================================")
    ...
    print("\n[+] Running Extraction...")
    if not run_script("scripts/extract_prefamily_child_excel_aligned.py"):
        print("[-] Extraction failed!")
        sys.exit(1)
        
    print("\n[+] Running Upload...")
    if not run_script("scripts/upload_child_sick_rates.py"):
        print("[-] Upload failed!")
        sys.exit(1)
    ...
    print("\n==================================================")
    print("  ★ CHILD SICK UPDATE COMPLETED SUCCESSFULLY! ★")
    print("==================================================")
