...
def load_cancer_final_v2():
    cancer_dir = os.path.join(base_path, r'insurance_data\0_popular\cancer')
    # 명칭을 '형'으로 통일하여 웹사이트와 100% 동기화
    files = {
        "비갱신형": 'cancer_non_renewable.csv',
        "갱신형": 'cancer_renewable.csv',
        "표적항암형": 'cancer_targeted.csv'
    }

    print("[*] 암보험 최종 동기화 적재 시작 (비갱신형/갱신형/표적항암형)...")
    ...
    for category_name, file_name in files.items():
        ...
        for p_name, pkg in packages.items():
            ...
            # 필터링
            if m_total < 5000 or m_total > 200000: continue
        ...
        if products_batch:
            ...
            print(f"[+] {category_name} 적재 완료")

if __name__ == "__main__":
    load_cancer_final_v2()
