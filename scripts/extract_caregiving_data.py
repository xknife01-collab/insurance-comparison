...
TARGET_DIR = r"C:\Users\zkfnt\Desktop\insurance-comparison-main\insurance-comparison-main\insurance_data\2_care\caregiving"
...
def extract_caregiving_data():
    ...
    for filename in files:
        ...
        for idx, row in df.iterrows():
            ...
            # Check if this row belongs to pure caregiving insurance (contains 간병/간병인/요양, excludes 치매/CDR/종신)
            care_keywords = ["간병", "간병인", "요양"]
            exclude_keywords = ["치매", "CDR", "종신"]
            
            is_care = any(k in product_name for k in care_keywords) or any(k in row_list[mapping.get("담보명(급부명)", 3)] for k in care_keywords)
            is_excluded = any(k in product_name for k in exclude_keywords)
            
            if is_care and not is_excluded:
    ...
    if not extracted_rows:
        print("No caregiving rows extracted!")
        return
