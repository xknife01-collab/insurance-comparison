...
TARGET_DIR = r"C:\Users\zkfnt\Desktop\insurance-comparison-main\insurance-comparison-main\insurance_data\5_savings\variable_term"

STANDARD_HEADERS = [
    "보험회사", "상품명", "구분", "담보명(급부명)", "지급사유", 
    "지급금액", "가입금액", "기준보험료", "가입보험료", "적용이율",
    "갱신구분", "판매채널", "기준일자", "상세안내", "연락처", "source_file", "file_type", "sub_type"
]
...
def process_files():
    ...
    for filename in sorted(files):
        ...
        for idx, row in df.iterrows():
            ...
            # Filter by keywords "변액" or "정기"
            is_matched = any(k in product_name for k in KEYWORDS)
            
            if is_matched:
                # Classify sub_type based on product name
                sub_type = "term_pure"
                pname_upper = product_name.upper()
                if any(k in pname_upper for k in ["경영인", "CEO"]):
                    sub_type = "term_ceo"
                elif "변액" in pname_upper:
                    if any(k in pname_upper for k in ["정기", "종신"]):
                        sub_type = "variable_term"
                    elif any(k in pname_upper for k in ["연금", "저축", "적립", "유니버셜"]):
                        sub_type = "variable_saving"
                    else:
                        sub_type = "variable_saving"
                else:
                    sub_type = "term_pure"
                ...
                for h in STANDARD_HEADERS:
                    if h == "source_file":
                        ...
                        ordered_part.append("html" if method.startswith("html") else "binary")
                    elif h == "sub_type":
                        ordered_part.append(sub_type)
                    else:
        ...
        if file_rows_count > 0:
            print(f"  - {filename} ({method}): Extracted {file_rows_count} variable/term rows")
            
    print(f"[*] Total variable/term rows extracted: {len(extracted_rows)}")
    ...
    if len(extracted_rows) > 0:
        ...
        print("[!] No variable or term insurance data found.")
